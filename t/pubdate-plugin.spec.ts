...
it('make pipe depends on config', () => {
  const items = parseSources('t/pubdate-plugin/two_articles.podlite')
  const config: PluginConfig = {
    plugin: pubdatePlugin(),
    includePatterns: '.*',
...
it('pubdatePlugin: extract articles', () => {
  const items = parseSources('t/pubdate-plugin/extract_articles.podlite')
  expect(items.length).toEqual(1)
...
it('pubdatePlugin: extract 2 pages', () => {
  const items = parseSources('t/pubdate-plugin/two_pages.podlite')
  expect(items.length).toEqual(1)
  const res = getPages(items[0])
  expect(res.length).toEqual(2)
  // check if all articles have pubdate and title, type
...
`,
  ...
  const items = [...parseSources('t/pubdate-plugin/subset2.*'), additinalItems]
